...
class Search extends Component {
    ...
    searchquery(userquery){
        ...
        if (userquery != "") { if (this.state.usersearch.title == null) { headertext = "No Result Found" } else{headertext = "Search result"} }
                ...
                renderItem={({ item }) => {
                    return (
                        <View style={{ flexDirection: 'column',flex: 1 , margin: 12 }} >
                            <TouchableOpacity onPress={() => this.props.navigation.navigate('WatchSearch', {
                                poster: item.image,
